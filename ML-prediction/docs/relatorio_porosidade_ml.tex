% Compilar neste diretorio: pdflatex relatorio_porosidade_ml.tex
%                         bibtex relatorio_porosidade_ml
%                         pdflatex relatorio_porosidade_ml.tex (x2)
...
% Nomenclatura fixa (definida na Metodologia): F03-4 = referencia; F06-1 = validacao entre colunas.
\newcommand{\dadosref}{conjunto de dados de referência (F03-4)}
\newcommand{\dadosval}{conjunto de dados de validação (F06-1)}
\newcommand{\Dadosref}{Conjunto de dados de referência (F03-4)}
\newcommand{\Dadosval}{Conjunto de dados de validação (F06-1)}
...
histogramas; fundamentação em \citealp{friedman2001greedy}, com variantes de
eficiência em espírito semelhante a \citealp{ke2017lightgbm} e implementação no
ecossistema de \citealp{pedregosa2011scikit}). A avaliação usa partição treino--teste ao longo da profundidade;
...
conjunto tabular pela profundidade mecânica registrada ao longo da coluna, mantendo o treino na fração rasa
...
preditor com base em Shapley; \citealp{lundberg2017unified}). Complementam o quadro, no conjunto de teste profundo,
o erro absoluto médio (MAE, do inglês \textit{mean absolute error}) e a raiz do
erro quadrático médio (RMSE, \textit{root mean squared error}). No \textbf{\dadosref}, sob partição treino--teste ao longo da profundidade e com as curvas
...
irrelevância física universal dessas grandezas. A leitura das ablações e do diagnóstico
multivariado no \dadosref\ permanece ancorada a esse protocolo intra-coluna.
Complementarmente, uma validação LOO entre quatro conjuntos de dados 7logs (tabela no texto)
indica encaixe elevado em cada poço retido, com heterogeneidade moderada entre colunas;
isso não substitui estudos alargados nem afirma independência geológica plena entre
esses quatro poços.
\end{abstract}
...
\dadosref\ considerado, dado o forte acoplamento dessas grandezas com AC e \(\rho_b\) e a
...
leitura de redundância entre preditores. Inclui-se ainda um contraste entre o
\dadosref\ e o \dadosval\ e uma validação LOO entre quatro conjuntos de dados 7logs, para
lembrar que a leitura entre contextos não se resume a \(R^2\) elevado num único intervalo
nem implica transferência homogênea entre colunas.
...
\begin{enumerate}
  ...
  elásticos?
  \item O mesmo protocolo, aplicado ao \dadosval, produz \(R^2\)
...
Utilizou-se um perfil tabular com \textbf{seis} preditores --- intervalo de trânsito (AC),
densidade aparente \(\rho_b\), impedância acústica (AI), velocidade \(V_p\),
raio gama (GR) e variável associada à argila (\(V_c\)) --- e \textbf{porosidade} como alvo.

As rotinas de leitura e filtragem usadas na preparação exploratória admitem esquemas
3logs, 6logs e 7logs; contudo, as inferências centrais deste relatório limitam-se a
\textbf{recortes tabulares 7logs completos} (em particular os dois conjuntos fixados
abaixo), sem pretender que conclusões obtidas nesse esquema se estendam automaticamente
a outros conjuntos de curvas sem repetir o protocolo.
...
\textbf{identificador de amostra} próprio (etiqueta da coluna ou série considerada).
Dois conjuntos fixam o texto central. O \textbf{\dadosref} concentra as figuras e
tabelas de diagnóstico multivariado (correlações, VIF, MI, SHAP, ablações). O
\textbf{\dadosval} designa o segundo conjunto tabular onde o mesmo protocolo
computacional é aplicado para confrontar métricas entre
colunas; \textbf{não} se confunde com um subconjunto de validação no sentido de
partição treino/validação \emph{dentro} da mesma coluna (essa partição continua a ser
apenas raso versus profundo em cada coluna). O vocabulário \textit{recorte} não implica,
por si só, o mesmo poço em duas profundidades nem litologia dominante idêntica entre
etiquetas distintas.

\subsection{Fluxo de processamento}

\subsubsection{Estimador: GBDT em histogramas}\label{subsec:gbdt_just}
O \textit{Gradient Boosting} sobre árvores de decisão (GBDT) constrói um conjunto de
árvores de forma \textbf{sequencial}: a cada etapa acrescenta-se uma árvore que
aproxima o gradiente da função de perda em relação à predição acumulada, de modo
que novas árvores corrigem principalmente os resíduos ainda mal explicados pelo
conjunto anterior, em vez de forçar uma única árvore muito profunda. A predição
final soma contribuições aditivas das árvores, na formulação clássica de
\textit{gradient boosting} para aproximação gulosa por árvores fracas
\citep{friedman2001greedy}.

Neste ensaio, o GBDT é usado como \textbf{regressor de referência} sobre dados
\textbf{tabulares} (linhas = profundidades, colunas = curvas e alvo), contexto em
que boosting com árvores costuma ser competitivo sem exigir especificação explícita
de termos não lineares ou de interações: partições recursivas capturam relações não
lineares e acoplamentos entre preditores sem suposição de linearidade forte entre
porosidade e logs, adequado quando se admite que a relação físico-estatística pode
desviar-se de um modelo linear simples.

A implementação adotada segue a variante \textbf{baseada em histogramas}
(\textit{histogram-based}): valores contínuos das entradas são discretizados em
intervalos (\textit{bins}) durante o treino, reduzindo o custo de procura de cortes
em colunas com muitas observações e tendendo a estabilizar o ajuste em bases
relativamente grandes frente a métodos que avaliam cada ponto de corte contínuo
isoladamente. Essa estratégia de discretização alinha-se a implementações de
\textit{boosting} de alta eficiência descritas na literatura \citep{ke2017lightgbm};
o ajuste numérico integra o ecossistema \textit{software} de aprendizado de máquina
documentado em \citep{pedregosa2011scikit}.

Por fim, \textbf{não} se impõe padronização \textit{z-score} obrigatória nas features
antes do GBDT: partições por limiar em árvores são invariantes a transformações
monótonas afim por coluna (mudança de unidade ou escala linear não altera a ordem
dos valores), ao contrário de regressões lineares ou de métodos guiados por
distâncias, nos quais a escala importa diretamente. Mantém-se apenas, como
\textbf{variante opcional}, o escalonamento das entradas para comparações com
modelos lineares auxiliares, preservando coerência com a natureza do estimador
principal e evitando etapas de pré-processamento desnecessárias para o bloco central
em árvores.
...
que distorceriam correlações; o terceiro impõe que o teste seja \textbf{contíguo em profundidade}
(um bloco contínuo ao longo do eixo vertical tabulado), o que aproxima o protocolo de um cenário em que se
...
Para \textbf{rastreabilidade} das execuções numéricas, alinhada à ordem da
Figura~\ref{fig:fluxo}, as etapas finais de diagnóstico multivariado (último bloco do
fluxograma) podem deixar, quando a infraestrutura o permitir, um \textbf{registro
auxiliar em formato JSON} com carimbo temporal em UTC e identificador curto da
revisão Git do código utilizado, de forma a associar tabelas e figuras ao estado do
software na data da rodagem. O mesmo tipo de registro pode ser produzido de forma
\textbf{opcional} após o ajuste do GBDT (bloco central da mesma figura), quando a
gravação desse metadado estiver habilitada na execução.
...
no estudo principal, a \(20\%\) dos registros mais profundos (em sensibilidades
...
Valores elevados indicam que \(x_j\) é quase combinação linear dos
outros preditores. Complementarmente, calcularam-se matrizes de correlação de
Pearson e de Spearman \textbf{no treino} (figuras do \dadosref), a informação mútua (MI, \textit{mutual information})
entre cada preditor e a porosidade \textbf{também no treino}, a importância por permutação da queda de
\(R^2\) no teste e um resumo SHAP (\textit{SHapley Additive exPlanations}; \citealt{lundberg2017unified}) obtido com
...
e o mesmo teste profundo. Sobre as \textbf{features elásticas} standardizadas do treino
(AC, \(\rho_b\), impedância e \(V_p\)), aplicou-se uma PCA apenas como leitura
geométrica: embora existam quatro colunas, os perfis variam \textbf{quase conjuntamente},
de modo que quase toda a dispersão conjunta do bloco se projeta em \textbf{poucas}
direções. Concretamente, as duas primeiras componentes principais explicam cerca de
\(99{,}9\%\) da variância acumulada nesse subconjunto (Figura~\ref{fig:pca_cumvar}). Isto interpreta-se como evidência
de \textbf{dimensão efetiva baixa}: o conjunto de estados elásticos visitados pela amostra
ocupa, aproximadamente, um \textbf{subespaço} de dimensão intrínseca pequena dentro de
\(\mathbb{R}^4\), em lugar de se comportarem como quatro sinais mutuamente
independentes no sentido estatístico do termo. Em consequência, espera-se
\textbf{redundância preditiva} entre essas curvas em modelos lineares ou em árvores
que particionam o espaço de entradas. O número de condição
(norma espectral) da matriz de correlação do treino é da ordem de \(10^7\), sinal de
matriz mal condicionada, coerente com VIF extremos quando o mesmo bloco é visto por
regressões auxiliares lineares.
...
entre conjuntos tabulares distintos nestes quatro conjuntos de dados 7logs. Os \(R^2\) no poço retido
situam-se entre \(0{,}960\) e cerca de \(0{,}9998\) (MAE baixo em todas as linhas), o que indica
transferência entre poços globalmente favorável neste conjunto, sem uniformidade estrita:
quando o teste é F02-1, o \(R^2\) é o mais baixo da tabela e o MAE o mais alto, enquanto as outras
três exclusões concentram-se ainda mais perto do teto numérico. Com apenas quatro recortes,
o LOO não substitui um estudo de campo exaustivo nem documenta grau de independência física
entre poços, semelhança instrumental ou ausência de acoplamento alvo--preditores. O contraste visual entre \dadosref\ e \dadosval\ na Figura~\ref{fig:f06}
...
No \dadosref, as quatro variantes apresentam desempenho praticamente
idêntico: a remoção de \(V_p\) ou de impedância, isolada ou conjunta, não
altera de forma substantiva MAE, RMSE nem \(R^2\). Já no \dadosval,
...
A comparação entre a primeira linha da tabela (F03-4: todos os preditores elásticos) e a
última (F06-1: validação; baseline completa) constitui apenas \textbf{contraste qualitativo} entre
duas séries tabulares sob o \textbf{mesmo} protocolo interno (treino no trecho raso,
teste no bloco profundo de 20\%); \textbf{não} corresponde ao protocolo LOO da
Tabela~\ref{tab:loo_well}, em que um único modelo treina na união de três colunas e
avalia exclusivamente na quarta.

\begin{table}[h]
\centering
\begin{tabular}{lccc}
\toprule
Cenário & MAE & RMSE & \(R^2\) \\
\midrule
F03-4: todos os preditores elásticos & 0{,}00053 & 0{,}00111 & 0{,}988 \\
F03-4: sem \(V_p\) & 0{,}00054 & 0{,}00111 & 0{,}988 \\
F03-4: sem impedância & 0{,}00054 & 0{,}00111 & 0{,}988 \\
F03-4: sem \(V_p\) e sem impedância & 0{,}00054 & 0{,}00111 & 0{,}988 \\
F06-1: validação (baseline completa) & 0{,}00222 & 0{,}00456 & 0{,}945 \\
\bottomrule
\end{tabular}
\caption{Desempenho no teste profundo (20\% inferior da coluna). As quatro primeiras linhas
referem-se ao \dadosref; a última, ao \dadosval. MAE~\eqref{eq:mae} e
...
\textbf{baseline completa} em cada coluna (referência e validação), isto é, o mesmo cenário da primeira
...
\toprule
Coluna (baseline completa) & IC 95\% MAE & IC 95\% RMSE & IC 95\% \(R^2\) \\
\midrule
...
Mantendo o mesmo GBDT no \dadosref, repetiu-se a partição com frações de teste
...
encaixe elevado no teste \textbf{não} depende de um único ponto de corte na grade considerada.
...
poços disponíveis (F02-1, F03-2, F03-4, F06-1). Na Tabela~\ref{tab:loo_well}, o \(R^2\) no
poço retido situa-se entre \(0{,}960\) e \(0{,}999\) (valor pontual máximo \(0{,}999845\))
e o MAE permanece baixo em todas as linhas: o quadro indica generalização entre poços
globalmente forte nestes quatro conjuntos de dados. A variação entre linhas ---
notadamente o contraste de F02-1 (\(R^2=0{,}960\), MAE \(=0{,}00311\)) com F03-2, F03-4 e
F06-1 --- mostra que \(R^2\) elevado \textbf{não} implica transferência estritamente homogênea
entre contextos distintos; qualifica um resultado globalmente favorável, sem sugerir
instabilidade grave nem substituir validação em conjuntos mais vastos ou em condições
de aquisição claramente independentes.
...
\bottomrule
\end{tabular}
\caption{Validação LOO entre quatro conjuntos de dados tabulares 7logs (treino na união dos outros três,
...
As Figuras~\ref{fig:f03vp} a~\ref{fig:f03svpai} mostram, para o \dadosref,
...
marcada em todas as variantes, o que é coerente com as quatro primeiras linhas da
Tabela~\ref{tab:metricas} (todas no F03-4, \(R^2\approx 0{,}988\) no teste profundo).
A Figura~\ref{fig:f06} transporta o mesmo tipo de painel para o \dadosval: observa-se maior dispersão relativa entre observado e previsto, coerente com
a \textbf{última linha} dessa mesma tabela --- a linha \textit{F06-1: validação (baseline completa)},
em que \(R^2=0{,}945\) (e MAE/RMSE superiores às linhas do F03-4). Isto não é uma ``queda''
de \(R^2\) ao longo do tempo nem um efeito das figuras 2--5: é apenas o contraste entre
dois ajustes \emph{independentes} (uma coluna versus outra) sob o mesmo protocolo de
partição raso/profundo. Distingue-se da Tabela~\ref{tab:loo_well}, que reporta métricas
do protocolo LOO (poço retido). O contraste visual com a Figura~\ref{fig:f06} reforça o
mesmo aviso: neste painel de figuras e nas linhas correspondentes da Tabela~\ref{tab:metricas},
cada coluna tem \textbf{o seu} modelo GBDT --- treino e teste limitados aos dados daquela
coluna (partição raso/profundo). Comparar F03-4 com F06-1 \textbf{não} significa aplicar
ao F06-1 o estimador já ajustado no F03-4; seriam precisos dados do poço-alvo no treino
ou um protocolo explícito de transferência. A exclusão de poço no treino, com métricas na
Tabela~\ref{tab:loo_well}, aproxima precisamente essa leitura entre conjuntos de dados distintos.

\begin{figure}[p]
\centering
\includegraphics[width=0.85\textwidth]{porosity_F03-4_com_Vp.png}
\caption{\Dadosref: porosidade observada e prevista com todos os preditores
...
\caption{\Dadosref: modelo sem \(V_p\).}
\label{fig:f03svp}
\end{figure}
...
\caption{\Dadosref: modelo sem impedância acústica.}
\label{fig:f03sai}
\end{figure}
...
\caption{\Dadosref: modelo apenas com AC, \(\rho_b\), GR e \(V_c\).}
\label{fig:f03svpai}
\end{figure}
...
\caption{\Dadosval: baseline completa.}
\label{fig:f06}
\end{figure}
...
como correlação forte deste bloco com a porosidade \textbf{no \dadosref}.
Formalmente, quatro curvas elásticas definem quatro coordenadas, mas o padrão de
acoplamento mostra que a amostra percorre apenas uma \textbf{faixa estreita} de
combinações possíveis: mudanças de AC acompanham variações coerentes de \(\rho_b\),
AI e \(V_p\), em vez de quatro fontes de variação ortogonais. Por \textbf{dimensão
efetiva baixa} entende-se precisamente isto --- o volume ``real'' ocupado pelos
dados no espaço das elásticas tem poucos graus de liberdade estatísticos, compatível
com a síntese por PCA no treino (Figura~\ref{fig:pca_cumvar}; duas componentes concentram cerca de \(99{,}9\%\) da
variância do bloco; Secção~\ref{subsec:featstudy}). Essa imagem ajuda a explicar por
que retirar \(V_p\) ou a impedância pode deixar o \(R^2\) quase inalterado: essas
grandezas funcionam, neste conjunto, como \textbf{reparametrizações} do mesmo eixo
dominante já informado por AC e \(\rho_b\), e não como eixos independentes.
...
\bottomrule
\end{tabular}
\caption{Fatores VIF aproximados no \dadosref (treino, bloco raso), calculados por~\eqref{eq:vif}.}
\label{tab:vif}
\end{table}
...
No \dadosref, sobre o mesmo conjunto 7logs já filtrado por linhas
...
de regressões lineares ordinárias (controle pela profundidade; controle adicional
...
das curvas). As tabelas seguintes consolidam os indicadores dessa etapa para o \dadosref.
...
de valores distintos e a mesma fração de linhas em empates (\dadosref).}
\label{tab:grvc_ties}
\end{table}
...
\toprule
Medida & Notação / controle & Valor \\
\midrule
Pearson (bruto) & \(\rho_{GR,Vc}\) & 0{,}993 \\
Spearman & \(\rho_S\) & 1{,}000 \\
Kendall & \(\tau\) & 1{,}000 \\
Parcial (bruto) & resíduos OLS; controle: profundidade & 0{,}993 \\
Parcial (postos) & resíduos OLS em postos médios; controle: posto(prof.) & 1{,}000 \\
Parcial (bruto) & resíduos OLS; controle: AC, \(\rho_b\), AI, \(V_p\) & 0{,}992 \\
Parcial (postos) & postos médios; controle: prof.\ e elásticos (postos) & 1{,}000 \\
...
OLS; controle \textbf{linear} apenas, sem equivalência a independência condicional geral).
...
mesma grade de quantização. A Tabela~\ref{tab:grvc_assoc} confirma uma relação
...
menos no esquema de controle linear adotado. Uma regressão isotônica crescente
...
quantifica a redundância monótona além das correlações clássicas. \textbf{No \dadosref, GR e \(V_c\)
...
Spearman e Kendall próximos de um indicam monotonia quase perfeita no \dadosref.}
...
elevados e próximos entre si para AC, \(\rho_b\), impedância e \(V_p\),
...
menores no mesmo ajuste MI. Isto reforça que, \textbf{no \dadosref}, o
bloco elástico compartilha quase o mesmo conteúdo informativo sobre o alvo, ao
passo que GR e \(V_c\) introduzem um eixo litológico mais fraco neste \dadosref, sem
ser nulo.
...
A Figura~\ref{fig:shap}, no mesmo modelo GBDT e no \dadosref, mostra que AC
...
\caption{Correlações de Pearson entre preditores e porosidade (\dadosref).}
\label{fig:pearson}
\end{figure}
...
\caption{Correlações de Spearman (\dadosref).}
\label{fig:spearman}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.85\textwidth]{study_F03-4_pca_cumulative_variance.png}
\caption{PCA no bloco elástico standardizado do \textbf{treino} (AC, \(\rho_b\), impedância e \(V_p\)):
fração de variância explicada por componente e variância acumulada (leitura geométrica; não substitui o modelo preditivo).}
...
indica que, \textbf{no \dadosref} e sob o protocolo treino/teste em profundidade,
...
(Figuras~\ref{fig:pearson} e~\ref{fig:spearman}), PCA cumulativa no mesmo bloco (Figura~\ref{fig:pca_cumvar}), VIF extremos na
...
parcial ainda elevada após controle linear pela profundidade ou pelo bloco elástico,
o que reforça que, neste \dadosref, GR e \(V_c\) não funcionam como preditores
...
do \dadosref. Daí a cautela ao extrapolar conclusões entre colunas distintas sem
protocolo de exclusão mútua no treino.

\paragraph{Contraste entre \dadosref\ e \dadosval.}
No \dadosref, após remoção de linhas incompletas, restam
\(n=1435\) profundidades entre \(856\) e \(1071\,\mathrm{m}\), com porosidade média
\(0{,}278\) e desvio-padrão \(0{,}014\). No \dadosval, \(n=576\)
...
O protocolo núcleo (partição raso/profundo por coluna) caracteriza \textbf{robustez interna}
ao conjunto de dados e ao modelo ajustado naquela coluna. A Tabela~\ref{tab:loo_well} acrescenta,
sobre quatro conjuntos de dados 7logs, evidência de encaixe elevado no poço retido em cada exclusão,
compatível com transferência favorável nesse conjunto finito, porém \textbf{não} uniforme
entre linhas nem substituto de domínio geológico alargado. O teste profundo permanece,
por construção, intra-coluna; a leitura das ablações e do diagnóstico multivariado nas
figuras centrais deve distinguir-se da extrapolação além do material tabular analisado
ou além dos quatro conjuntos de dados percorridos no LOO.

\section{Conclusões}\label{sec:conclusoes}

No \dadosref, o modelo atinge \(R^2\) elevado no teste profundo e mantém
...
\textbf{no modelo ajustado e neste \dadosref}, AC emerge como o principal carregador de
...
O \dadosval lembra que a generalização espacial ou entre contextos
...
\(R^2\) entre o \dadosref\ e o \dadosval\ não é compatível com intervalos sobrepostos sob a
...
preditores no \dadosref.

O protocolo principal no \dadosref\ caracteriza robustez interna à coluna; a
Tabela~\ref{tab:loo_well} acrescenta validação LOO entre quatro conjuntos de dados, com encaixe
elevado em cada poço retido e heterogeneidade moderada (F02-1 mais exigente que os
demais). Como próximos passos, \textbf{recomenda-se}: (i) alargar validação entre poços
para mais colunas ou bases independentes quando dados estiverem disponíveis, mantendo
exclusão mútua no treino; (ii) repetir o mesmo protocolo de diagnóstico em múltiplos
conjuntos de dados, para verificar estabilidade da leitura de redundância; (iii) quando possível,
substituir ou complementar a porosidade-alvo por referência menos acoplada às curvas
elásticas usadas como entrada.

As conclusões sobre ablações, VIF, MI e SHAP no \dadosref constituem evidência
\textbf{intra-coluna} naquele protocolo em profundidade. A síntese LOO aplica-se aos
quatro conjuntos de dados indicados e não se estende, sem novo desenho, a outros poços,
profundidades ou esquemas de curvas.
