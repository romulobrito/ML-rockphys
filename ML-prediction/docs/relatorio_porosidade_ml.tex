...
por coerência com modelos baseados em árvores (a padronização permanece disponível
no código apenas para comparações opcionais com modelos lineares); calculam-se
...
explicador de árvore aplicado ao modelo GBDT ajustado. As correlações no conjunto completo
(antes do corte) são exportadas como ficheiro auxiliar para EDA, mas não substituem as
matrizes do treino nas figuras reportadas. A permutação e o SHAP reutilizam o mesmo modelo
...
construção, \textbf{intra-coluna} (corte vertical num único ficheiro). O ensaio
\textit{leave-one-well-out} da Tabela~\ref{tab:loo_well} aproxima exclusão mútua
entre ficheiros, mas com apenas quatro poços não substitui um estudo de campo
exaustivo: os \(R^2\) por linha variam e um caso (F02-1 como teste) desvia
substancialmente dos demais. O contraste entre recortes na Figura~\ref{fig:f06}
compara qualidade de ajuste interno a cada coluna, não transferência aprendida
numa coluna e testada na outra. Por fim, \(R^2\) alto pode refletir acoplamento fí sico e redundância
...
Mantendo o mesmo GBDT e o ficheiro F03-4, repetiu-se a partição com frações de teste
...
Com \texttt{max\_depth} \(\in \{4,6,8\}\) e demais hiperparâmetros fixos, o \(R^2\) no
...
\subsubsection{Exclusão mútua entre poços (\textit{leave-one-well-out})}
Para aproximar a crítica de generalização \textbf{entre} colunas, treinou-se o mesmo tipo
de GBDT na união de três ficheiros 7logs e testou-se no quarto, percorrendo os quatro
poços disponíveis (F02-1, F03-2, F03-4, F06-1). O \(R^2\) no poço retido varia
substancialmente (Tabela~\ref{tab:loo_well}): permanece elevado quando o teste é
F03-2, F03-4 ou F06-1, mas cai para cerca de \(0{,}96\) quando o teste é F02-1, o que
reforça que ``\(R^2\) alto'' não é garantia automática de transferência entre contextos
litológicos e instrumentais distintos.
...
\bottomrule
\end{tabular}
\caption{LOO entre quatro ficheiros 7logs: treino na união dos outros três, teste no poço da linha.
...
Com monotonia empí rica quase perfeita entre GR e \(V_c\), os \(p\)-valores
...
unitário e erro de reconstrução desprezível na precisão numérica exportada, o que
...
\bottomrule
\end{tabular}
\caption{Importância por permutação no teste profundo (25 repetições). O ficheiro
exportado inclui a coluna booleana \texttt{likely\_redundant\_or\_noise} quando a
queda média de \(R^2\) não é positiva.}
\label{tab:perm_r2}
\end{table}
