...
gravando em cada execução um ficheiro JSON (\texttt{experiment.json}) com carimbo temporal em UTC
...
pode registar o mesmo tipo de ficheiro quando ativado por opção de linha de comandos).
...
exaustivo: os \(R^2\) por linha oscilam num intervalo estreito (todos acima de \(0{,}96\));
o caso em que o poço F02-1 fica reservado para teste apresenta o menor valor desse conjunto,
sem deixar, contudo, de corresponder a ajuste elevado em termos absolutos. O contraste entre recortes na Figura~\ref{fig:f06}
...
poço retido situa-se entre \(0{,}960\) e \(0{,}999\): os quatro valores são elevados e
diferem sobretudo na terceira casa decimal, com o mínimo quando o teste é F02-1. A
dispersão é modesta em escala absoluta, mas suficiente para mostrar que o coeficiente
\textbf{não} é constante entre poços neste protocolo. Isto reforça que ``\(R^2\) alto'' não
equivale, por si só, a \textbf{mesma} qualidade de transferência entre contextos litológicos
e instrumentais distintos.
...
As conclusões quantitativas deste documento constituem evidência \textbf{intra-coluna}
no recorte e no protocolo indicados; não se estendem, por si sós, a outros poços,
profundidades ou esquemas de curvas sem replicação explícita do mesmo desenho.
