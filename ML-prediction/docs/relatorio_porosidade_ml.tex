...
Para rastreabilidade computacional, o módulo partilhado \texttt{run\_metadata.py}
é invocado em \texttt{feature\_relationship\_study.py} e em \texttt{gr\_vc\_rank\_dependence.py},
gravando em cada execução um registo auxiliar em JSON (\texttt{experiment.json}) com carimbo temporal em UTC
e identificador curto da revisão Git quando o repositório está acessível a partir do diretório
de trabalho, alinhando números e figuras desses blocos ao estado do código (o baseline de porosidade
pode registar o mesmo tipo de saída JSON quando ativado por opção de linha de comandos).
