...
diagnóstico de colinearidade e leitura crí tica das métricas}
\author{}
\date{\today}
...
\begin{abstract}
Trata-se do registro analí tico de um ensaio de regressão para estimar porosidade
...
ablações de preditores e diagnóstico multivariado: correlações de Pearson
e de Spearman, fator de inflação da variância (VIF), informação mútua
(MI) entre cada preditor e a porosidade, importância por permutação do
...
tal como exportadas, \(V_p\) e a impedância acústica (AI) não acrescentam
...
irrelevância fí sica universal dessas grandezas. Os resultados devem ser lidos como
...
rocha. Em colunas perfiladas, relações empí ricas entre porosidade e
...
Os achados de ablação não implicam irrelevância fí sica de \(V_p\) ou de AI,
...
O objetivo deste texto é documentar, com rigor modesto porém explí cito, um
...
\textbf{identificador de amostra} próprio (nome de exportação do arquivo de
dados). O \textbf{recorte principal} corresponde ao identificador F03-4; o
...
mantendo o trecho mais raso para treino; aplica-se padronização \textit{z-score}
no treino; estima-se o modelo GBDT já definido no resumo; calculam-se
MAE~\eqref{eq:mae}, RMSE~\eqref{eq:rmse} e \(R^2\)~\eqref{eq:r2} no teste, com variantes
por ablação;
por fim, executa-se o bloco de diagnóstico multivariado (correlações, VIF,
MI, permutação e SHAP) sobre o mesmo conjunto de teste. A avaliação por
...
\node[box, below=of s] (p) {Padronização\\e modelo GBDT};
...
contí nuo na profundidade, o que aproxima o protocolo de um cenário em que se
prevê porosidade abaixo de uma profundidade de corte usando apenas informação
mais rasa. O quarto bloco aplica a padronização \textit{z-score} aprendida no
treino e estima o GBDT, passo necessário porque AC, \(\rho_b\), AI e \(V_p\) têm
ordens de grandeza distintas; as predições nesse bloco alimentam as curvas da
...
redundância e sensibilidade: as matrizes de correlação resumem acoplamento
linear ou monótono; o VIF mede quasi-colinearidade num sistema linear auxiliar; a
MI mede dependência estatí stica (não necessariamente linear) de cada preditor
com a porosidade; a permutação mede o quanto o \(R^2\) degrada ao destruir
informação de um preditor de cada vez; o SHAP decompõe predições em
contribuições aditivas locais. Assim, a Figura~\ref{fig:fluxo} não dispensa a
...
independentes em perfil quase contí nuo: o teste corresponde ao trecho mais
profundo (20\% das observações), onde a extrapolação a partir de padrões
rasos é mais exigente do que numa partição aleatória i.i.d.\ (independentes
e identicamente distribuí das entre observações).

Foram treinadas variantes do mesmo modelo com subconjuntos de preditores: todas as
variáveis elásticas disponí veis; sem \(V_p\); sem impedância; sem \(V_p\) e
...
aplicou-se \textbf{bootstrap pareado}: sorteiam-se \(n\) í ndices do teste com
...
(\texttt{numpy}, semente \(0\)). Os quantis empí ricos \(2{,}5\%\) e \(97{,}5\%\)
...
face aos demais preditores, com
...
Pearson e de Spearman, a informação mútua (MI, \textit{mutual information})
entre cada preditor e a porosidade, a importância por permutação da queda de
\(R^2\) no teste e um resumo SHAP (\textit{SHapley Additive exPlanations}) obtido com
explicador de árvore aplicado ao modelo GBDT ajustado. Estes passos usam o mesmo
conjunto de teste profundo já definido, de modo que as leituras permanecem
...
sem corrigir deslocamentos instrumentais. A limitação metodológica central
é a ausência de validação com exclusão de poço: o protocolo é, por
construção, \textbf{intra-coluna}. O teste profundo não substitui validação
entre poços com exclusão mútua de treino: o contraste entre recortes, mostrado
na Figura~\ref{fig:f06}, compara qualidade de ajuste interno a cada coluna, não
transferência aprendida numa coluna e testada na outra. Por fim, \(R^2\) alto pode refletir acoplamento fí sico e redundância
estatí stica, não independência informacional entre preditores. A informação
mútua estimada por \texttt{mutual\_info\_regression} (mesma pilha numérica do
...
F03-4 e \(n=116\) no F06-1). Limites: quantis empí ricos \(2{,}5\%\) e \(97{,}5\%\)
das réplicas.}
...
\(8{,}5\times 10^5\) e \(1{,}3\times 10^5\) não devem ser interpretadas como diferença
...
AC & $8{,}5 \times 10^5$ \\
$\rho_b$ & $8{,}5 \times 10^5$ \\
Impedância & $1{,}3 \times 10^5$ \\
$V_p$ & $1{,}3 \times 10^5$ \\
GR & $7{,}9 \times 10^1$ \\
$V_c$ & $7{,}8 \times 10^1$ \\
\bottomrule
\end{tabular}
\caption{Fatores VIF aproximados no recorte principal, calculados por~\eqref{eq:vif}.}
\label{tab:vif}
\end{table}
...
(\textit{min} e \textit{max} do \texttt{rankdata}) informa se a dependência seria
artefato numérico ou se permanece quase determiní stica.

No recorte principal, sobre o mesmo conjunto 7logs já filtrado por linhas
completas, calcularam-se estatí sticas de empates, correlações de Pearson,
...
OLS). Com monotonia empí rica quase perfeita entre GR e \(V_c\), os \(p\)-valores
...
\bottomrule
\end{tabular}
\caption{Sensibilidade ao desempate: Pearson entre postos definidos com
\texttt{rankdata(..., method)} (\textit{scipy}); valores fora da diagonal diferem
só na séxta casa decimal.}
...
é compatível com \(V_c\) construí do a partir de GR (ou inversamente) com a
...
menos no esquema de controlo linear adotado. \textbf{No recorte principal, GR e \(V_c\)
não devem ser tratados como eixos independentes de informação:} a evidência
sugere que um deles é, na prática, quase inteiramente reconstruí vel a partir do
...
ficam na casa de \(5{,}5\) nats (arredondamento à segunda casa), enquanto GR e
\(V_c\) permanecem perto de \(0{,}50\), isto é, magnitudes cerca de dez vezes
...
\bottomrule
\end{tabular}
\caption{MI estimada no conjunto completo antes da partição (mesma corrida que
gera as figuras de correlação). Valores arredondados; ver limitações na
...
à escala do ruí do de permutação. GR exibe média ligeiramente negativa
(\(\approx -9\times 10^{-4}\)), compatível com flutuação estatí stica em torno
de zero quando o preditor é redundante; \(V_c\) registra queda média nula na
precisão exportada, coerente com o acoplamento quase determiní stico a GR já
documentado.
...
equivalem a ausência fí sica de conteúdo informativo, mas sim a redundância
...
\bottomrule
\end{tabular}
\caption{Importância por permutação no teste profundo (25 repetições).}
\label{tab:perm_r2}
\end{table}
...
do ajuste, não como atribuição causal fí sica.
...
quando AC e \(\rho_b\) já estão disponí veis com a qualidade relativa destas
curvas. Não se trata de desvalorizar medidas fí sicas centrais: o experimento não
implica irrelevância fí sica de \(V_p\) ou de AI, mas sim ausência de contribuição
...
redundantes face a AC e \(\rho_b\), sem afastar a sua relevância fí sica em outros
...
\textbf{recomenda-se}: (i) validação \textit{leave-one-well-out} ou equivalente com
