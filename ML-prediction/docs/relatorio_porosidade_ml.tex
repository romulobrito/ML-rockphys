...
multivariado no recorte principal permanece ancorada a esse protocolo intra-coluna.
...
recortes verticais distintos e uma validação LOO entre quatro conjuntos de dados 7logs, para
lembrar que a leitura entre contextos não se resume a \(R^2\) elevado num único intervalo
nem implica transferência homogénea entre colunas.
...
\textbf{recortes tabulares 7logs completos} (em particular F03-4 e F06-1 no texto principal),
sem pretender que conclusões obtidas nesse esquema se estendam automaticamente a outros
conjuntos de curvas sem repetir o protocolo.
...
entre poços, semelhança instrumental ou ausência de acoplamento alvo--preditores. O contraste entre recortes na Figura~\ref{fig:f06}
...
encaixe elevado no teste \textbf{não} depende de um único ponto de corte na grelha considerada.
...
F06-1 --- mostra que \(R^2\) elevado \textbf{não} implica transferência estritamente homogénea
...
A Figura~\ref{fig:f06} transporta o mesmo tipo de painel para o recorte de contraste
(F06-1): observa-se maior dispersão relativa entre observado e previsto, coerente com
a \textbf{última linha} dessa mesma tabela --- a linha \textit{Recorte de contraste (baseline completa)},
...
ao recorte e ao modelo ajustado naquela coluna. A Tabela~\ref{tab:loo_well} acrescenta,
sobre quatro recortes 7logs, evidência de encaixe elevado no poço retido em cada exclusão,
...
O protocolo principal no recorte F03-4 caracteriza robustez interna à coluna; a
...
recortes, para verificar estabilidade da leitura de redundância; (iii) quando possível,
substituir ou complementar a porosidade-alvo por referência menos acoplada às curvas
elásticas usadas como entrada.

As conclusões sobre ablações, VIF, MI e SHAP no recorte principal constituem evidência
\textbf{intra-coluna} naquele protocolo em profundidade. A síntese LOO aplica-se ao
